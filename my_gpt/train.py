    ...
    try:
        for _ in range(n_epochs):
            ...
            for train_batch, target_batch in train_loader:
                ...
                if global_step > 0 and global_step % print_sample_period == 0:
                    generate_and_print_sample(model, tokenizer, device, start_context)

                if global_step > 0 and global_step % save_ckpt_period == 0:
...
def get_lr(step: int, warmup_steps: int, max_lr: float) -> float:
    ...
    """
    min_lr = 0.1 * max_lr
    max_steps = 5000 * warmup_steps  # 10_000
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=10_000,
        help="Iterations between saving model checkpoints",
    )
    ...
    parser.add_argument("--wandb_project", type=str, default="gpt-train", help="Weights & Biases project name")
    parser.add_argument(
        "--wandb_entity",
        type=str,
        default="beatwad",
    ...
    parser.add_argument("--wandb_name", type=str, default="wandb_run", help="Weights & Biases run name")
    ...
    for index, file_path in tqdm(enumerate(all_files)):
        ...
        global_step, tokens_seen = train(
            ...
            save_ckpt_period=args.save_ckpt_period,
            model_dir=model_dir,
        )
